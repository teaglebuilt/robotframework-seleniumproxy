    ...
    def filter_headers(self, headers):
        ...
        if 'Accept-Encoding' in headers:
            ...
            filtered_encodings = [x for x in re.split(
                r',\s*', ae) if x in permitted_encodings]
...
class ProxyAwareHTTPConnection(HTTPConnection):
    ...
    def __init__(self, proxy_config, netloc, *args, **kwargs):
        ...
        if self.proxied:
            _, self.proxy_username, self.proxy_password, self.proxy_host = proxy_config.get(
                'http')
            self.custom_authorization = proxy_config.get(
    ...
    def request(self, method, url, body=None, headers=None, *, encode_chunked=False):
        ...
        if self.proxied:
            ...
            headers.update(proxy_auth_headers(self.proxy_username,
                                              self.proxy_password, self.custom_authorization))
...
class ProxyAwareHTTPSConnection(HTTPSConnection):
    ...
    def __init__(self, proxy_config, netloc, *args, **kwargs):
        ...
        if self.proxied:
            _, proxy_username, proxy_password, proxy_host = proxy_config.get(
                'https')
